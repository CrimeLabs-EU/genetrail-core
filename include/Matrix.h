...
#ifndef GENETRAILCORE_MATRIX_H
#define GENETRAILCORE_MATRIX_H

namespace GeneTrail::Core {
        class GENETRAILCORE_EXPORT Matrix {
        public:
            ...
            value_type set(index_type i, index_type j, value_type v) {
                if (i >= rows()) { throw InvalidIndex(i, rows()); }

                if (j >= cols()) { throw InvalidIndex(i, cols()); }
