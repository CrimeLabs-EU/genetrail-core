...
namespace GeneTrail::Core {
    ...
    void GenericFile::openForReading_(const std::string &path) {
        ...
        switch (compressor) {
            ...
            case Compressor::NONE:
                // should never be reached under any condition
                assert(false);
                break;
        }
    ...
    void GenericFile::openForWriting_(const std::string &path, std::ios_base::openmode m) {
        if (path == "stdout") {
            out_strm_ = &std::cout;
            return;
        } else if (path == "stdout") {
        ...
        if (compressor == Compressor::NONE) { return; }
    ...
    GenericFile::Compressor GenericFile::getCompressor_(const std::string &path) const {
    ...
    GenericFile::operator bool() const { return isValid_(); }

    bool GenericFile::operator!() const { return !isValid_(); }

    bool GenericFile::isReading() const { return mode_ == FileOpenMode::READ; }

    bool GenericFile::isWriting() const { return mode_ == FileOpenMode::WRITE || mode_ == FileOpenMode::APPEND; }

    FileOpenMode GenericFile::mode() const { return mode_; }

    bool GenericFile::isValid_() const { return (in_strm_ && *in_strm_) || (out_strm_ && *out_strm_); }
