...
namespace GeneTrail::Core {
    ...
    bool Category::contains(const std::string &id) const { return contains(database_->index(id)); }

    bool Category::contains(size_t i) const { return container_.find(i) != container_.end(); }

    bool Category::insert(const std::string &id) { return insert(database_->index(id)); }
    ...
    bool Category::operator<(const Category &o) const {
        return std::lexicographical_compare(container_.begin(), container_.end(), o.container_.begin(), o.container_.end());
    }
    ...
    Category Category::intersect(const std::string &name, const Category &a, const Category &b) {
        Category result(a.database_);
        result.setName(name);

        if (a.database_ != b.database_) { throw std::invalid_argument("EntityDatabases not compatible"); }
        std::set_intersection(a.container_.begin(), a.container_.end(), b.container_.begin(), b.container_.end(),
                              std::inserter(result.container_, std::end(result.container_)));
        return result;
    }

    Category Category::combine(const std::string &name, const Category &a, const Category &b) {
        Category result(a.database_);
        result.setName(name);
